...
public class ServiceSpecificationApiController extends AbstractApiController implements ServiceSpecificationApi {
        ...
        return checkingMono
                ...
                .onErrorMap(throwable ->
                        new ServiceCatalogException(
                                String.format("Was not able to validate service service specification %s", serviceSpecification.getId()),
                                throwable,
    ...
    private void validateCharacteristicSpecifications(ServiceSpecification serviceSpecification) {
        if (serviceSpecification.getSpecCharacteristic() == null || serviceSpecification.getSpecCharacteristic().isEmpty()) {
            // early exit
            return;
        }
        List<String> charIds = serviceSpecification.getSpecCharacteristic().stream().map(CharacteristicSpecification::getId).toList();
        if (charIds.size() != new HashSet<>(charIds).size()) {
            throw new ServiceCatalogException("Duplicate characteristic ids are not allowed.", ServiceCatalogExceptionReason.INVALID_DATA);
        }
        serviceSpecification.getSpecCharacteristic()
                ...
                .findFirst()
                .ifPresent(rId -> {
                    throw new ServiceCatalogException(String.format("Referenced characteristic %s does not exist.", rId), ServiceCatalogExceptionReason.INVALID_DATA);
                });
    }

    private Mono<ServiceSpecification> validateEntitySpecRelationship(ServiceSpecification serviceSpecification, Mono<ServiceSpecification> checkingMono) {
        if (serviceSpecification.getEntitySpecRelationship() == null || serviceSpecification.getEntitySpecRelationship().isEmpty()) {
            // early exit
            return checkingMono;
        }
        ...
        if (!associationSpecificationRefs.isEmpty()) {
            checkingMono = Mono.zip(checkingMono, getCheckingMono(serviceSpecification, List.of(associationSpecificationRefs)), (p1, p2) -> serviceSpecification);
        }
        return checkingMono;
    }
    ...
    private Mono<ServiceSpecification> validateFeatureSpecification(ServiceSpecification serviceSpecification, Mono<ServiceSpecification> checkingMono) {
        ...
        if (featureSpecIds.size() != new HashSet<>(featureSpecIds).size()) {
            throw new ServiceCatalogException("Duplicate featureSpec ids are not allowed.", ServiceCatalogExceptionReason.INVALID_DATA);
        }
    ...
    private Mono<ServiceSpecification> validateFeatureSpecCharacteristics(ServiceSpecification serviceSpecification, Mono<ServiceSpecification> checkingMono, List<String> featureSpecIds) {
        ...
        List<String> featureSpecCharIds = featureSpecCharList.stream().map(FeatureSpecificationCharacteristic::getId).toList();
        if (featureSpecCharIds.size() != new HashSet<>(featureSpecCharIds).size()) {
            throw new ServiceCatalogException("Duplicate featureSpecChar ids are not allowed.", ServiceCatalogExceptionReason.INVALID_DATA);
        }
        ...
        if (!resourceSpecificationRefs.isEmpty()) {
            checkingMono = Mono.zip(checkingMono, getCheckingMono(serviceSpecification, List.of(resourceSpecificationRefs)), (p1, p2) -> serviceSpecification);
        }
        return checkingMono;
    }

    private void validateFeatureSpecRelationships(ServiceSpecification serviceSpecification, List<String> featureSpecIds) {
        ...
        featureSpecificationRelationships
                ...
                .findFirst()
                .ifPresent(rId -> {
                    throw new ServiceCatalogException(String.format("Referenced feature %s does not exist.", rId), ServiceCatalogExceptionReason.INVALID_DATA);
                });
    }

    private Mono<ServiceSpecification> validateFeatureSpecConstraints(ServiceSpecification serviceSpecification, Mono<ServiceSpecification> checkingMono) {
        List<ConstraintRef> constraintRefs = serviceSpecification
                ...
                .flatMap(List::stream)
                .toList();
        return Mono.zip(checkingMono, getCheckingMono(serviceSpecification, List.of(constraintRefs)), (p1, p2) -> serviceSpecification);
    }

    private void validateFeatureSpecRelRefs(List<String> featureSpecIds, List<String> featureSpecCharIds, FeatureSpecificationCharacteristicRelationship fscr) {
        if (!featureSpecIds.contains(fscr.getFeatureId())) {
            throw new ServiceCatalogException(
                    String.format("Feature %s referenced by feature spec char does not exist.", fscr.getFeatureId()),
                    ServiceCatalogExceptionReason.INVALID_DATA);
        }
        if (!featureSpecCharIds.contains(fscr.getCharacteristicId())) {
            throw new ServiceCatalogException(
                    String.format("Characteristic %s referenced by feature spec char does not exist.", fscr.getFeatureId()),
                    ServiceCatalogExceptionReason.INVALID_DATA);
        }
    }
    ...
    private Mono<ServiceSpecification> getCheckingMono(ServiceSpecification serviceSpecification) {
        ...
        return getCheckingMono(serviceSpecification, references)
                .onErrorMap(throwable ->
                        new ServiceCatalogException(
                                String.format("Was not able to validate service specification %s", serviceSpecification.getId()),
                                throwable,
                                ServiceCatalogExceptionReason.INVALID_RELATIONSHIP));
    }
    ...
    @Override
    public Mono<HttpResponse<ServiceSpecificationVO>> patchServiceSpecification(@NonNull String id, @NonNull ServiceSpecificationUpdateVO serviceSpecificationUpdateVO) {
        // non-ngsi-ld ids cannot exist.
        if (!IdHelper.isNgsiLdId(id)) {
            throw new ServiceCatalogException("Did not receive a valid id, such service specification cannot exist.", ServiceCatalogExceptionReason.NOT_FOUND);
        }
    ...
    public Mono<HttpResponse<ServiceSpecificationVO>> retrieveServiceSpecification(@NonNull String id, @Nullable String fields) {
        return retrieve(id, ServiceSpecification.class)
                .switchIfEmpty(Mono.error(new ServiceCatalogException("No such service specification exists.", ServiceCatalogExceptionReason.NOT_FOUND)))
                .map(tmForumMapper::map)
                .map(HttpResponse::ok);
    }
}
