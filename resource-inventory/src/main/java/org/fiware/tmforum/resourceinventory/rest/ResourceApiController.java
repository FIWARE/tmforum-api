...
public class ResourceApiController extends AbstractApiController implements ResourceApi {
    ...
    public Mono<HttpResponse<ResourceVO>> createResource(@NonNull ResourceCreateVO resourceCreateVO) {
        Resource resource = tmForumMapper.map(
                tmForumMapper.map(resourceCreateVO, IdHelper.toNgsiLd(UUID.randomUUID().toString(), Resource.TYPE_RESOURCE)));
    ...
    private Mono<Resource> getCheckingMono(Resource resource) {
        ...
        Optional.ofNullable(resource.getResourceSpecification()).ifPresent(resourceSpecificationRef -> references.add(List.of(resourceSpecificationRef)));
        ...
        if (resource.getActivationFeature() != null && !resource.getActivationFeature().isEmpty()) {
            ...
            if (!featureConstraintsCheckingMonos.isEmpty()) {
                Mono<Resource> featureConstraintsCheckingMono = Mono.zip(featureConstraintsCheckingMonos, p -> resource);
                checkingMono = Mono.zip(featureConstraintsCheckingMono, checkingMono, (p1, p2) -> resource);
            }
        }
        return checkingMono
                .onErrorMap(throwable ->
                        new ResourceInventoryException(
                                String.format("Was not able to create resource %s", resource.getId()),
                                throwable,
                                ResourceInventoryExceptionReason.INVALID_RELATIONSHIP));
    }
    ...
    private void validateInternalCharacteristicRefs(Characteristic characteristic, List<Characteristic> characteristics) {
        ...
        if (charIds.size() != new HashSet<>(charIds).size()) {
            throw new ResourceInventoryException(
                    String.format("Duplicate characteristic ids are not allowed: %s", charIds), ResourceInventoryExceptionReason.INVALID_DATA);
        }
        ...
        if (characteristic.getCharacteristicRelationship() != null) {
            characteristic.getCharacteristicRelationship()
                    ...
                    .ifPresent(missingId -> {
                        throw new ResourceInventoryException(
                                String.format("Referenced characteristic %s does not exist", missingId), ResourceInventoryExceptionReason.INVALID_DATA);
                    });
        }
    }
    ...
    private void validateInternalFeatureRefs(Feature feature, Resource resource) {
        ...
        if (featureIds.size() != new HashSet<>(featureIds).size()) {
            throw new ResourceInventoryException(String.format("Duplicate feature ids are not allowed: %s", featureIds), ResourceInventoryExceptionReason.INVALID_DATA);
        }
        if (feature.getFeatureRelationship() != null) {
            feature.getFeatureRelationship()
                    ...
                    .findFirst()
                    .ifPresent(missingId -> {
                        throw new ResourceInventoryException(String.format("Referenced feature %s does not exist", missingId), ResourceInventoryExceptionReason.INVALID_DATA);
                    });
        }
        if (feature.getFeatureCharacteristic() != null) {
            feature.getFeatureCharacteristic()
                    .forEach(characteristic -> validateInternalCharacteristicRefs(characteristic, feature.getFeatureCharacteristic()));
        }
    }
    ...
    @Override
    public Mono<HttpResponse<List<ResourceVO>>> listResource(@Nullable String fields, @Nullable Integer offset, @Nullable Integer limit) {
    ...
    @Override
    public Mono<HttpResponse<ResourceVO>> patchResource(@NonNull String id, @NonNull ResourceUpdateVO resourceUpdateVO) {
        // non-ngsi-ld ids cannot exist.
        if (!IdHelper.isNgsiLdId(id)) {
            throw new ResourceInventoryException("Did not receive a valid id, such resource cannot exist.", ResourceInventoryExceptionReason.NOT_FOUND);
        }
    ...
    public Mono<HttpResponse<ResourceVO>> retrieveResource(@NonNull String id, @Nullable String fields) {
        return retrieve(id, Resource.class)
                .switchIfEmpty(Mono.error(new ResourceInventoryException("No such resource exists.", ResourceInventoryExceptionReason.NOT_FOUND)))
                .map(tmForumMapper::map)
                .map(HttpResponse::ok);
    }
}
